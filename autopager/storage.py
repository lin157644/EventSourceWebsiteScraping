...
class Storage(object):
    ...
    def get_Xy(self, language=None, validate=True, contain_button=True, contain_position=False, file_type='T',
               scaled_page='normal', verbose=False):
        X, y, scaled_pages = [], [], []
        if verbose:
        ...
        for row in self.iter_records(contain_button, file_type, language):
            ...
            if verbose:
                print(
                    f"Finish: Get Page {row['File Name']} (Encoding: {row['Encoding']})records ... (len: {len(yseq)})")
            X.append(xseq)
            y.append(yseq)
