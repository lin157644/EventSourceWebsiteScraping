...
def get_links_loose(sel, contain_button):
    """
    ...
    """
    return parsel.SelectorList(_get_links(sel, contain_button))


def _get_links(sel, contain_button):
    if contain_button is True:
        for el in sel:
...
def get_every_button_and_a(sel):
    return sel.xpath(".//a|.//button")


def get_every_leaf_node(sel):
    return sel.xpath(".//*[not(child::*)]")


def get_xseq_yseq(root, selectors, validate=True, contain_button=False):
    """
    ...
    """
    if contain_button is True:
        xseq = get_every_button_and_a(root)
    else:
        #         xseq = get_links(root)
        xseq = get_every_a(root)
    ...
    for label, selector in selectors.items():
        if not selector or selector == 'N/A':
            continue
        sel_links = set(get_links_loose(root.css(selector), contain_button).extract())
        if validate and not sel_links:
            print(root.css(selector).extract())
